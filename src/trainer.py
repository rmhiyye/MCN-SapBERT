...
from src.model import BertWithCustomNNClassifier
...
class n2c2Trainer(object):
        def __init__(self, args, train_dataset=None, test_dataset=None, validation_dataset=None, train_ref_dataset=None, test_ref_dataset=None):
                ...
                # self.basenorm = NeuralNetwork(config.embbed_size).to(self.device)
                self.model = BertWithCustomNNClassifier(config.model_name, config.embbed_size).to(self.device)
                ...
                if self.args.do_train:
                        ...
                        self.val_loader = DataLoader(val_set, batch_size=config.batch_size, shuffle=True)
        ...
        def train(self):
                ...
                for epoch in range(self.config.epochs):
                        ...
                        if (epoch + 1) % self.config.loss_step == 0:
                                print("-------------------------------")
        ...
        def inference(self):
                ...
                self.model.eval()
                X_pred = self.map_mention(self.test_dataset)
                cui_encode = self.map_cui(self.test_ref_dataset)
        ...
        def candidates_rank(self, X_pred, cui_encode):
                ...
                # Create a matrix to hold the CUI vectors.
                CUIVectorMatrix = np.squeeze(np.stack(list(cui_encode.values())))

                # Calculate distance matrix
                scoreMatrix = cdist(X_pred, CUIVectorMatrix, 'cosine')

                cui_keys = list(cui_encode.keys())
        ...
        def encoder(self, dataset, ref):
                ...
                with torch.no_grad():
                        for i, idx in tqdm(enumerate(dataset.keys()), total=len(dataset)):
                                ...
                                X[i] = self.tokenize(dataset[idx]['mention'])
                                y[i] = self.tokenize(ref[cui[i]])
        ...
        def map_cui(self, ref):
                ...
                with torch.no_grad():
                        for cui in ref.keys():
                            cui_encode[cui] = self.model(self.tokenize(ref[cui])).cpu()
                            if self.embbed_size == None:
                                self.embbed_size = len(cui_encode[cui][0])
                return cui_encode # Returns a dictionnary containing the embedding of each concept in the ontology.
        ...
        def map_mention(self, dataset):
                ...
                with torch.no_grad():
                        for i, idx in tqdm(enumerate(dataset.keys()), total=len(dataset)):
                                ...
                                X_pred[i] = self.model(tokenized_mention).cpu().numpy()

                return X_pred # Returns a matrix containing the embedding of each mention in the dataset.
